...
template <class E> auto pdist_seuclidean_1D(const xt::xexpression<E> &expr) {
  const auto &mat = expr.derived_cast();
...
template <class E> auto pdist_seuclidean_ND(const xt::xexpression<E> &expr) {
  const auto &mat = expr.derived_cast();
  auto var = xt::variance(mat, /*axes=*/{0}, /*ddof=*/1);
...
template <class E> auto pdist_seuclidean(const xt::xexpression<E> &expr) {
  ...
  std::size_t m_entries = n_rows * (n_rows - 1) / 2;
  if (!var.dimension()) {
    return pdist_seuclidean_1D(expr);
  } else {
    return pdist_seuclidean_ND(expr);
  }
}
